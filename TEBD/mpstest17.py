...
def randomMPSOBC(N,chi,d,real=False):
    ...
    if real:
        A = [randomReal((d,chi))]
        for i in xrange(N-2):
            A.append(randomReal((chi,d,chi)))
        A.append(randomReal((chi,d)))
    else:
...
##    show(A[-1].shape,"A[-1].shape")
##    print(N)
##    for a in A:
##        print(a.shape)
    a = np.reshape(a,(A[-1].shape[-1],d,a.size/d/A[-1].shape[-1]))
    A.append(a)
    A.append(c)
...
def allEqual(N):
    state = np.ones(2**N,dtype=complex)
    state = state/np.linalg.norm(state)
    mps = getMPSOBC(state,5)
    return mps
