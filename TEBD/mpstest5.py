...
def fidelityMPS(A,B):
    """ Fidelity of two MPS """
    return 1. - innerProduct(A,B)*innerProduct(B,A)/innerProduct(A,A)/innerProduct(B,B)

def fidelity(a,b):
    """ Fidelity of two states """
    return 1.- np.inner(a,np.conj(b))*np.inner(b,np.conj(a))/np.inner(a,np.conj(a))/np.inner(b,np.conj(b))
